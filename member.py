...
class Trainer(object):
    ...
    def __init__(self, model_class, optimizer_class, loss_function, batch_size, train_data, device, verbose = False):
        ...
        self.train_data = DataLoader(train_data, batch_size, True)
...
class Member(mp.Process):
    ...
    def __str__(self):
        return f"{get_datetime_string()} - epoch {self.trainer.epochs} ({self.trainer.steps}/{len(self.trainer.train_data)}) - member {self.id}"
